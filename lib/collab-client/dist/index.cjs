...
const collabClientKey = new pm.PluginKey('bangle.dev/collab-client');
const collabMonitorKey = new pm.PluginKey('bangle.dev/collabMonitorKey');
var EventType;
(function (EventType) {
...
function applySteps(view, payload, logger) {
    ...
    const steps = (payload.steps ? payload.steps : []).map((j) => pm.Step.fromJSON(view.state.schema, j));
    const clientIDs = payload.clientIDs ? payload.clientIDs : [];
    if (steps.length === 0) {
        logger('no steps', payload, 'version', prosemirrorCollab.getVersion(view.state));
        return;
    }
    try {
        const tr = prosemirrorCollab.receiveTransaction(view.state, steps, clientIDs)
    ...
    logger('after apply version', prosemirrorCollab.getVersion(view.state));
    return true;
}
...
function applyDoc(view, doc, version, oldSelection) {
    ...
    const prevSelection = view.state.selection instanceof pm.TextSelection
    ...
    if (selection) {
        ...
        if (from >= tr.doc.content.size) {
            tr = tr.setSelection(pm.Selection.atEnd(tr.doc));
        }
        else {
            tr = tr.setSelection(pm.Selection.near(tr.doc.resolve(from)));
        }
    }
    try {
...
function replaceDocument(state, serializedDoc, version) {
    ...
    serializedDoc instanceof pm.Node
    ...
    tr.setSelection(pm.Selection.atStart(tr.doc));
...
function isOutdatedVersion() {
    return (state) => {
        ...
        return (typeof serverVersion === 'number' && prosemirrorCollab.getVersion(state) < serverVersion);
    };
}
...
class InitState extends CollabBaseState {
    ...
    async runAction({ clientInfo, signal, view }) {
        ...
        if (typeof clientInfo.warmupTime === 'number') {
            await utils.sleep(clientInfo.warmupTime);
            if (signal.aborted) {
                return;
            }
        }
...
class ReadyState extends CollabBaseState {
    ...
    async runAction({ signal, view }) {
        ...
        if (!signal.aborted) {
            ...
            else if (prosemirrorCollab.sendableSteps(view.state)) {
                this.dispatch(signal, view.state, view.dispatch, {
                    type: EventType.Push,
...
class PushState extends CollabBaseState {
    ...
    async runAction({ clientInfo, signal, view }) {
        ...
        const steps = prosemirrorCollab.sendableSteps(view.state);
        if (!steps) {
            this.dispatch(signal, view.state, view.dispatch, {
                type: EventType.Ready,
        ...
        const response = await clientCom.pushEvents({
            ...
            version: prosemirrorCollab.getVersion(view.state),
    ...
    async runAction({ clientInfo, logger, signal, view }) {
        ...
        if (response.ok) {
            ...
            if (success === false) {
                this.dispatch(signal, view.state, view.dispatch, {
                    type: EventType.PushPullError,
                    payload: { failure: collabComms.CollabFail.ApplyFailed },
...
const handleErrorStateAction = async ({ clientInfo, view, logger, signal, collabState, }) => {
    ...
    switch (failure) {
        case collabComms.CollabFail.InvalidVersion: {
            utils.abortableSetTimeout(() => {
                if (!signal.aborted) {
                    collabState.dispatch(signal, view.state, view.dispatch, {
        ...
        case collabComms.CollabFail.IncorrectManager: {
            collabState.dispatch(signal, view.state, view.dispatch, {
                type: EventType.Fatal,
                payload: {
        ...
        case collabComms.CollabFail.HistoryNotAvailable: {
            collabState.dispatch(signal, view.state, view.dispatch, {
                type: EventType.Fatal,
                payload: {
        ...
        case collabComms.CollabFail.DocumentNotFound: {
        ...
        // 409
        case collabComms.CollabFail.OutdatedVersion: {
        ...
        // 500
        case collabComms.CollabFail.ManagerDestroyed: {
            utils.abortableSetTimeout(() => {
                if (!signal.aborted) {
                    if (collabState instanceof PushPullErrorState) {
                        collabState.dispatch(signal, view.state, view.dispatch, {
        ...
        case collabComms.CollabFail.ApplyFailed: {
            if (collabState instanceof PushPullErrorState) {
                utils.abortableSetTimeout(() => {
                    if (!signal.aborted) {
                        collabState.dispatch(signal, view.state, view.dispatch, {
        ...
        case collabComms.CollabFail.ManagerUnresponsive: {
            utils.abortableSetTimeout(() => {
                if (!signal.aborted) {
                    if (collabState instanceof PushPullErrorState) {
                        collabState.dispatch(signal, view.state, view.dispatch, {
...
function collabClientPlugin({ requestTimeout, clientID, collabMessageBus, docName, managerId, cooldownTime, userId, warmupTime, }) {
    const logger = (state) => (...args) => {
        var _a;
        return log(`${clientID}:version=${prosemirrorCollab.getVersion(state)}:debugInfo=${(_a = collabClientKey.getState(state)) === null || _a === void 0 ? void 0 : _a.collabState.debugInfo}`, ...args);
    };
    return [
        new pm.Plugin({
            key: collabClientKey,
            ...
            view(view) {
                ...
                let clientCom = new collabComms.ClientCommunication({
                    clientId: clientID,
                    managerId,
                    docName,
 ...
 */
function pluginsFactory({ requestTimeout, clientID = 'client-' + utils.uuid(), collabMessageBus, docName, managerId = collabComms.DEFAULT_MANAGER_ID, cooldownTime = 100, warmupTime = 0, }) {
    const userId = 'user-' + clientID;
    return [
        prosemirrorCollab.collab({
            clientID,
        }),
        collabClientPlugin({
            requestTimeout,
...
exports.collabClient = collabExtension;
