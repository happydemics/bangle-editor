...
function insertParagraph(tokens) {
    return tokens.flatMap((token) => {
        if (['th_open', 'td_open'].includes(token.type)) {
            ...
            return [token, new Token__default["default"]('paragraph_open', 'p', 1)];
        }
        if (['th_close', 'td_close'].includes(token.type)) {
            return [token, new Token__default["default"]('paragraph_close', 'p', -1)];
        }
        return token;
    });
}
    ...
    function processToken(tokens, index) {
        ...
        children
            ...
            if (inlineToken.children == null) {
                inlineToken.children = [new Token__default["default"]('text', '', 0)];
            }
