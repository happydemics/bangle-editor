...
function pluginsFactory$e({ markdownShortcut = true, keybindings = defaultKeys$b, } = {}) {
    return ({ schema }) => {
        const type = utils.getNodeType(schema, name$e);
        return [
            markdownShortcut && pm.wrappingInputRule(/^\s*>\s$/, type),
            keybindings &&
                pm.keymap(utils.createObject([
                    [keybindings.wrapIn, wrapInBlockquote()],
                    [keybindings.moveUp, pmCommands.moveNode(type, 'UP')],
                    [keybindings.moveDown, pmCommands.moveNode(type, 'DOWN')],
                    [keybindings.emptyCopy, pmCommands.copyEmptyCommand(type)],
                    [keybindings.emptyCut, pmCommands.cutEmptyCommand(type)],
...
function queryIsBlockquoteActive() {
    return (state) => {
        const type = utils.getNodeType(state, name$e);
        return Boolean(utils.findParentNodeOfType(type)(state.selection));
    };
}
function wrapInBlockquote() {
    return utils.filter((state) => !queryIsBlockquoteActive()(state), (state, dispatch, _view) => {
        const type = utils.getNodeType(state, name$e);
        return pm.wrapIn(type)(state, dispatch);
    });
}
function insertEmptyParaAbove$1() {
    const isInBlockquote = queryIsBlockquoteActive();
    return (state, dispatch, view) => {
        const para = utils.getParaNodeType(state);
        return utils.filter(isInBlockquote, utils.insertEmpty(para, 'above', true))(state, dispatch, view);
    };
}
function insertEmptyParaBelow$1() {
    ...
    return (state, dispatch, view) => {
        const para = utils.getParaNodeType(state);
        return utils.filter(isInBlockquote, utils.insertEmpty(para, 'below', true))(state, dispatch, view);
    };
}
...
const getTypeFromSchema$5 = (schema) => {
    ...
    utils.assertNotUndefined(markType, `markType ${name$d} not found`);
    return markType;
};
...
function pluginsFactory$d({ markdownShortcut = true, keybindings = defaultKeys$a, } = {}) {
    return ({ schema }) => {
        ...
        return [
            ...
            markdownShortcut &&
                utils.markPasteRule(/(?:^|\s)((?:__)((?:[^__]+))(?:__))/g, type),
            markdownShortcut &&
                utils.markInputRule(/(?:^|\s)((?:__)((?:[^__]+))(?:__))$/, type),
            markdownShortcut &&
                utils.markInputRule(/(?:^|\s)((?:\*\*)((?:[^*]+))(?:\*\*))$/, type),
            keybindings &&
                pm.keymap(utils.createObject([[keybindings.toggleBold, toggleBold()]])),
        ];
    };
}
function toggleBold() {
...
const isNodeTodo = (node, schema) => {
    return (node.type === utils.getNodeType(schema, 'listItem') &&
...
const setTodoCheckedAttr = (tr, schema, node, pos) => {
    if (node.type === utils.getNodeType(schema, 'listItem') &&
 ...
 */
const removeTodo = utils.filter([isSelectionParentBulletList, (state) => todoCount(state).todos !== 0], (state, dispatch) => {
    const { schema } = state;
    let tr = state.tr;
...
function wrappingInputRuleForTodo(regexp, getAttrs) {
    return new pm.InputRule(regexp, function (state, match, start, end) {
        const nodeType = utils.getNodeType(state, 'listItem');
        var attrs = getAttrs instanceof Function ? getAttrs(match) : getAttrs;
        var tr = state.tr.delete(start, end);
        var $start = tr.doc.resolve(start), range = $start.blockRange(), wrapping = range && pm.findWrapping(range, nodeType, attrs);
        if (!wrapping) {
            return null;
        }
        ...
        if (before &&
            before.type === utils.getNodeType(state, 'bulletList') &&
            pm.canJoin(tr.doc, start - 1) &&
            before.lastChild &&
...
function siblingsAndNodesBetween(state, callback) {
    ...
    const range = $from.blockRange($to, (node) => node.childCount > 0 &&
        node.firstChild.type === utils.getNodeType(schema, 'listItem'));
    if (!range) {
        return;
    }
...
function isSelectionParentBulletList(state) {
    ...
    return (fromNode &&
        fromNode.type === utils.getNodeType(state, 'bulletList') &&
        endNode &&
        endNode.type === utils.getNodeType(state, 'bulletList'));
}
...
function liftListItem(type, state, selection, tr) {
    ...
    let listItem = type;
    if (!listItem) {
        listItem = utils.getNodeType(state, 'listItem');
    }
    ...
    if (end < endOfList) {
        tr.step(new pm.ReplaceAroundStep(end - 1, endOfList, end, endOfList, new pm.Slice(pm.Fragment.from(listItem.create(undefined, range.parent.copy())), 1, 0), 1, true));
        range = new pm.NodeRange(tr.doc.resolve($from.pos), tr.doc.resolve(endOfList), range.depth);
    }
    return tr.lift(range, pm.liftTarget(range)).scrollIntoView();
}
    ...
    tr.doc.nodesBetween(from, to, (node, pos) => {
        if (!lifted && node.type === listItem && pos > from) {
            ...
            while (listDepth > rootListDepth + 2) {
                ...
                const sel = new pm.TextSelection(start, end);
...
function liftSelectionList(type, state, tr) {
    ...
    for (let i = listCol.length - 1; i >= 0; i--) {
        ...
        if (start.depth > 0) {
            ...
            if (range) {
                tr.lift(range, utils.getListLiftTarget(type, state.schema, start));
            }
        }
    }
    return tr;
...
const canOutdent = (type) => (state) => {
    ...
    const { paragraph } = state.schema.nodes;
    if (state.selection instanceof utils.GapCursorSelection) {
        return parent.type === listItem;
    }
    return (parent.type === paragraph && utils.hasParentNodeOfType(listItem)(state.selection));
};
/**
...
const isInsideListItem = (type) => (state) => {
    ...
    return (utils.hasParentNodeOfType(listItem)(state.selection) &&
...
function toggleList(listType, itemType, todo) {
    return (state, dispatch, view) => {
        ...
        if (!fromNode ||
            ...
            const listItem = itemType ? itemType : utils.getNodeType(state, 'listItem');
...
function toggleListCommand(listType, todo = false) {
    ...
    const setTodoListTr = (schema) => (tr) => {
        ...
        const canBeTodo = (node, parentNode) => node.type === utils.getNodeType(schema, 'listItem') &&
            (parentNode === null || parentNode === void 0 ? void 0 : parentNode.type) === utils.getNodeType(schema, 'bulletList');
    ...
    return function (state, dispatch, view) {
        ...
        state = view.state;
        const { $from, $to } = state.selection;
        const isRangeOfSingleType = utils.isRangeOfType(state.doc, $from, $to, listType);
        if (isInsideList(state, listType) && isRangeOfSingleType) {
            ...
            const tr = utils.sanitiseSelectionMarksForWrapping(state, listType);
            if (tr && dispatch) {
                dispatch(tr);
                state = view.state;
            }
            // Wraps selection in list
            return wrapInList(listType)(state, todo ? utils.extendDispatch(dispatch, setTodoListTr(state.schema)) : dispatch);
        }
    };
}
function wrapInList(nodeType, attrs) {
    return pm.autoJoin(pm.wrapInList(nodeType, attrs), (before, after) => before.type === after.type && before.type === nodeType);
}
function liftListItems() {
    return function (state, dispatch) {
        ...
        tr.doc.nodesBetween($from.pos, $to.pos, (node, pos) => {
            ...
            if (node.isTextblock) {
                const sel = new pm.NodeSelection(tr.doc.resolve(tr.mapping.map(pos)));
                const range = sel.$from.blockRange(sel.$to);
                if (!range ||
                    ![utils.getNodeType(state, 'listItem')].includes(sel.$from.parent.type)) {
                    return false;
                }
                const target = range && pm.liftTarget(range);
...
function adjustSelectionInList(doc, selection) {
    ...
    if (endPos === startPos) {
        return new pm.TextSelection(doc.resolve(startPos));
    }
    return new pm.TextSelection(doc.resolve(startPos), doc.resolve(endPos));
}
    ...
    return function indentListCommand(state, dispatch) {
        ...
        if (isInsideListItem(listItem)(state)) {
            ...
            if (canSink(initialIndentationLevel, state)) {
                pm.sinkListItem(listItem)(state, utils.extendDispatch(dispatch, handleTodo(state.schema)));
            }
            return true;
        }
        return false;
...
function outdentList(type) {
    return function (state, dispatch, view) {
        ...
        if (dispatch && view) {
            ...
            let tr = state.tr;
            for (const { node, pos } of utils.flatten(grandParent, false)) {
        ...
        const composedCommand = utils.compose(mergeLists(listItem, range), // 2. Check if I need to merge nearest list
        pm.liftListItem)(listItem);
...
const backspaceKeyCommand = (type) => (state, dispatch, view) => {
    return pm.chainCommands(
    // if we're at the start of a list item, we need to either backspace
    // directly to an empty list item above, or outdent this node
    utils.filter([
        isInsideListItem(type),
        utils.isEmptySelectionAtStart,
        // list items might have multiple paragraphs; only do this at the first one
        utils.isFirstChildOfParent,
        canOutdent(type),
    ], pm.chainCommands(deletePreviousEmptyListItem(type), outdentList(type))), 
    // if we're just inside a paragraph node (or gapcursor is shown) and backspace, then try to join
    // the text to the previous list item, if one exists
    utils.filter([utils.isEmptySelectionAtStart, canToJoinToPreviousListItem], joinToPreviousListItem(type)))(state, dispatch, view);
};
    ...
    return (state, dispatch, view) => {
        ...
        if (selection.empty) {
            ...
            if (wrapper && wrapper.type === listItem) {
                ...
                const wrapperHasContent = utils.hasVisibleContent(wrapper);
                if (utils.isNodeEmpty(node) && !wrapperHasContent) {
                    ...
                    if (isNodeTodo(grandParent, state.schema) &&
                        ...
                        return outdentList(utils.getNodeType(state, 'listItem'))(state, dispatch, view);
                    }
                    else {
                        return outdentList(listItem)(state, dispatch, view);
                    }
                }
                else if (!codeBlock || !utils.hasParentNodeOfType(codeBlock)(selection)) {
...
function splitListItem(itemType, splitAttrs) {
    return function (state, dispatch) {
        ...
        grandParent.content.content.length <= 1 &&
            ...
            if (dispatch) {
                let wrap = pm.Fragment.empty;
                const keepItem = $from.index(-1) > 0;
                // Build a fragment containing empty versions of the structure
                // from the outer list item to the parent node of the cursor
                for (let d = $from.depth - (keepItem ? 1 : 2); d >= $from.depth - 3; d--) {
                    wrap = pm.Fragment.from($from.node(d).copy(wrap));
                }
                // Add a second list item with an empty default start node
                wrap = wrap.append(pm.Fragment.from(itemType.createAndFill()));
                const tr$1 = state.tr.replace($from.before(keepItem ? undefined : -1), $from.after(-3), new pm.Slice(wrap, keepItem ? 3 : 2, 2));
                tr$1.setSelection(pm.Selection.near(tr$1.doc.resolve($from.pos + (keepItem ? 3 : 2))));
...
function joinToPreviousListItem(type) {
    return (state, dispatch) => {
        ...
        const isGapCursorShown = state.selection instanceof utils.GapCursorSelection;
        const $cutPos = isGapCursorShown ? state.doc.resolve($from.pos + 1) : $from;
        let $cut = utils.findCutBefore($cutPos);
        if (!$cut) {
            return false;
        }
        ...
        if ($cut.nodeBefore &&
            ...
            if ($cut.nodeAfter &&
                ...
                let { tr } = state;
                if (isGapCursorShown) {
                    const nodeBeforePos = utils.findPositionOfNodeBefore(tr.selection);
                    if (typeof nodeBeforePos !== 'number') {
                        return false;
                    }
                    // append the paragraph / codeblock / heading to the list node
                    const list = $cut.nodeBefore.copy($cut.nodeBefore.content.append(pm.Fragment.from(listItem.createChecked({}, $cut.nodeAfter))));
                    tr.replaceWith(nodeBeforePos, $from.pos + $cut.nodeAfter.nodeSize, list);
                }
                else {
                    // take the text content of the paragraph and insert after the paragraph up until before the the cut
                    tr = tr.step(new pm.ReplaceAroundStep($lastNode.pos, $cut.pos + $cut.nodeAfter.nodeSize, $cut.pos + 1, $cut.pos + $cut.nodeAfter.nodeSize - 1, state.tr.doc.slice($lastNode.pos, $cut.pos), 0, true));
                }
...
function deletePreviousEmptyListItem(type) {
    return (state, dispatch) => {
        ...
        const $cut = utils.findCutBefore($from);
...
function moveEdgeListItem(type, dir = 'UP') {
    ...
    const command = (state, dispatch, view) => {
        ...
        const grandParent = utils.findParentNode((node) => utils.validListParent(node.type, state.schema.nodes))(state.selection);
        const parent = utils.findParentNodeOfType(listItem)(state.selection);
        ...
        if (isDown) {
            ...
            let uncle = utils.validPos(uncleNodePos, state.doc) && state.doc.nodeAt(uncleNodePos);
        ...
        const newTr = utils.safeInsert(nodeToInsert, insertPos)(tr);
    ...
    return utils.filter([isItemAtEdge], command);
}
...
function pluginsFactory$c({ markdownShortcut = true, todoMarkdownShortcut = true, keybindings = defaultKeys$9, } = {}) {
    return ({ schema }) => {
        const type = utils.getNodeType(schema, name$c);
        return [
            keybindings &&
                pm.keymap(utils.createObject([
            ...
            markdownShortcut &&
                pm.wrappingInputRule(/^\s*([-+*])\s$/, type, undefined, (_str, node) => {
...
function toggleBulletList() {
    const handleBulletLists = (state, dispatch, view) => toggleList(utils.getNodeType(state, 'bulletList'), utils.getNodeType(state, 'listItem'))(state, dispatch, view);
    return pm.chainCommands(removeTodo, handleBulletLists);
}
function toggleTodoList() {
    const fallback = (state, dispatch, view) => toggleList(utils.getNodeType(state, 'bulletList'), utils.getNodeType(state, 'listItem'), true)(state, dispatch, view);
    return pm.chainCommands(setTodo, fallback);
}
function queryIsBulletListActive() {
    return (state) => {
        return pmCommands.parentHasDirectParentOfType(utils.getNodeType(state, 'listItem'), [
            utils.getNodeType(state, 'bulletList'),
        ])(state);
    };
}
...
const getTypeFromSchema$4 = (schema) => {
    ...
    utils.assertNotUndefined(markType, `markType ${name$b} not found`);
    return markType;
};
...
function pluginsFactory$b({ markdownShortcut = true, escapeAtEdge = true, keybindings = defaultKeys$8, } = {}) {
    return ({ schema }) => {
        ...
        return [
            markdownShortcut && utils.markPasteRule(/(?:`)([^`]+)(?:`)/g, type),
            markdownShortcut && utils.markInputRule(/(?:`)([^`]+)(?:`)$/, type),
            keybindings &&
                pm.keymap(utils.createObject([[keybindings.toggleCode, pm.toggleMark(type)]])),
            escapeAtEdge &&
                pm.keymap({
                    ArrowRight: utils.filter(escapeFilters, moveRight),
                    ArrowLeft: utils.filter(escapeFilters, moveLeft),
                }),
        ];
    };
}
...
var moveLeft = (state, dispatch) => {
    ...
    if (!insideCode && atRightEdge) {
        const tr = state.tr.setSelection(pm.Selection.near(state.doc.resolve($cursor.pos - 1)));
        if (dispatch) {
...
function pluginsFactory$a({ markdownShortcut = true, keybindings = defaultKeys$7, } = {}) {
    return ({ schema }) => {
        const type = utils.getNodeType(schema, name$a);
        return [
            markdownShortcut && pm.textblockTypeInputRule(/^```$/, type),
            keybindings &&
                pm.keymap(utils.createObject([
                    [keybindings.toCodeBlock, pm.setBlockType(type)],
                    [keybindings.moveUp, pmCommands.moveNode(type, 'UP')],
                    [keybindings.moveDown, pmCommands.moveNode(type, 'DOWN')],
                    [
                        keybindings.insertEmptyParaAbove,
                        utils.filter(queryIsCodeActiveBlock(), utils.insertEmpty(utils.getParaNodeType(schema), 'above', false)),
                    ],
                    [
                        keybindings.insertEmptyParaBelow,
                        utils.filter(queryIsCodeActiveBlock(), utils.insertEmpty(utils.getParaNodeType(schema), 'below', false)),
                    ],
                ])),
        ];
    };
...
function queryIsCodeActiveBlock() {
    return (state) => {
        const type = utils.getNodeType(state, name$a);
        return Boolean(utils.findParentNodeOfType(type)(state.selection));
    };
}
...
function pluginsFactory$9({ keybindings = defaultKeys$6 } = {}) {
    return ({ schema }) => {
        const type = utils.getNodeType(schema, name$9);
        const command = pm.chainCommands(pm.exitCode, (state, dispatch) => {
            if (dispatch) {
        ...
        return [
            keybindings && pm.keymap(utils.createObject([[keybindings.insert, command]])),
        ];
    };
}
...
const defaultKeys$5 = {
    ...
    emptyCut: 'Mod-x',
    insertEmptyParaAbove: 'Mod-Shift-Enter',
    jumpToStartOfHeading: utils.browser.mac ? 'Ctrl-a' : 'Ctrl-Home',
    jumpToEndOfHeading: utils.browser.mac ? 'Ctrl-e' : 'Ctrl-End',
    insertEmptyParaBelow: 'Mod-Enter',
...
const checkIsInHeading = (state) => {
    const type = utils.getNodeType(state, name$8);
    return utils.findParentNodeOfType(type)(state.selection);
};
...
function pluginsFactory$8({ markdownShortcut = true, keybindings = defaultKeys$5, } = {}) {
    return ({ schema, specRegistry }) => {
        ...
        return [
            keybindings &&
                pm.keymap({
                    ...
                    ...utils.createObject([
                        [keybindings['moveUp'], pmCommands.moveNode(type, 'UP')],
                        [keybindings['moveDown'], pmCommands.moveNode(type, 'DOWN')],
                        [keybindings['jumpToStartOfHeading'], pmCommands.jumpToStartOfNode(type)],
                        [keybindings['jumpToEndOfHeading'], pmCommands.jumpToEndOfNode(type)],
                        [keybindings['emptyCopy'], pmCommands.copyEmptyCommand(type)],
                        [keybindings['emptyCut'], pmCommands.cutEmptyCommand(type)],
            ...
            ...(markdownShortcut ? levels : []).map((level) => pm.textblockTypeInputRule(new RegExp(`^(#{1,${level}})\\s$`), type, () => ({
                level,
            }))),
        ];
    };
...
function toggleHeading(level = 3) {
    return (state, dispatch) => {
        if (queryIsHeadingActive(level)(state)) {
            const para = utils.getParaNodeType(state);
            return pm.setBlockType(para)(state, dispatch);
        }
        return pm.setBlockType(utils.getNodeType(state, name$8), { level })(state, dispatch);
    };
}
function queryIsHeadingActive(level) {
    return (state) => {
        const match = utils.findParentNodeOfType(utils.getNodeType(state, name$8))(state.selection);
        if (!match) {
            return false;
        }
        const { node } = match;
...
function collapseHeading() {
    return (state, dispatch) => {
        ...
        let tr = state.tr.replaceWith(start, end, utils.getNodeType(state, name$8).createChecked({
            ...match.node.attrs,
            collapseContent: fragment.toJSON(),
        }, match.node.content));
        if (state.selection instanceof pm.TextSelection) {
            tr = tr.setSelection(pm.TextSelection.create(tr.doc, state.selection.from));
        }
        if (dispatch) {
            dispatch(tr);
        }
...
function uncollapseHeading() {
    return (state, dispatch) => {
        ...
        const frag = pm.Fragment.fromJSON(state.schema, match.node.attrs['collapseContent']);
        let tr = state.tr.replaceWith(match.pos, match.pos + match.node.nodeSize, pm.Fragment.fromArray([
            utils.getNodeType(state, name$8).createChecked({
                ...match.node.attrs,
...
function insertEmptyParaAbove() {
    return utils.filter(checkIsInHeading, (state, dispatch, view) => {
        const para = utils.getParaNodeType(state);
        return utils.insertEmpty(para, 'above', false)(state, dispatch, view);
    });
}
function insertEmptyParaBelow() {
    return utils.filter(checkIsInHeading, (state, dispatch, view) => {
        const para = utils.getParaNodeType(state);
        return utils.insertEmpty(para, 'below', false)(state, dispatch, view);
    });
}
...
function uncollapseAllHeadings() {
    return (state, dispatch) => {
        ...
        for (const { node, pos } of collapsibleNodes) {
            let baseFrag = pm.Fragment.fromJSON(state.schema, flattenFragmentJSON(node.attrs['collapseContent']));
            tr = tr.replaceWith(offset + pos, offset + pos + node.nodeSize, pm.Fragment.fromArray([
                utils.getNodeType(state, name$8).createChecked({
                    ...node.attrs,
                    collapseContent: null,
                }, node.content),
...
function listCollapsedHeading(state) {
    return utils.findChildren(state.doc, (node) => node.type === utils.getNodeType(state, name$8) &&
        Boolean(node.attrs['collapseContent']), false);
}
function listCollapsibleHeading(state) {
    return utils.findChildren(state.doc, (node) => node.type === utils.getNodeType(state, name$8), false);
}
...
function pluginsFactory$7({ markdownShortcut = true } = {}) {
    return ({ schema }) => {
        const type = utils.getNodeType(schema, name$7);
        return [
            markdownShortcut &&
                new pm.InputRule(/^(?:---|___\s|\*\*\*\s)$/, (state, match, start, end) => {
                    if (!match[0]) {
                        return null;
                    }
                    ...
                    return insertParaAfter
                        ? utils.safeInsert(utils.getParaNodeType(state).createChecked(), tr.mapping.map($para.after()))(tr)
                        : tr;
                }),
        ];
    };
...
function pluginsFactory$6({ handleDragAndDrop = true, acceptFileType = 'image/*', createImageNodes = defaultCreateImageNodes, } = {}) {
    return ({ schema }) => {
        const type = utils.getNodeType(schema, name$6);
        return [
            new pm.InputRule(/!\[(.+|:?)]\((\S+)(?:(?:\s+)["'](\S+)["'])?\)/, (state, match, start, end) => {
            ...
            handleDragAndDrop &&
                new pm.Plugin({
                    key: new pm.PluginKey(name$6 + '-drop-paste'),
                    props: {
                        handleDOMEvents: {
                            drop(view, _event) {
                                const event = _event;
                                ...
                                createImageNodes(files, utils.getNodeType(view.state, name$6), view).then((imageNodes) => {
...
function addImagesToView(view, pos, imageNodes) {
    for (const node of imageNodes) {
        const { tr } = view.state;
        let newTr = utils.safeInsert(node, pos)(tr);
        if (newTr === tr) {
            continue;
        }
        view.dispatch(newTr);
...
const updateImageNodeAttribute = (attr = {}) => (state, dispatch) => {
    if (!(state.selection instanceof pm.NodeSelection) || !state.selection.node) {
        return false;
    }
    const { node } = state.selection;
    if (node.type !== utils.getNodeType(state, name$6)) {
        return false;
    }
    if (dispatch) {
...
const getTypeFromSchema$3 = (schema) => {
    ...
    utils.assertNotUndefined(markType, `markType ${name$5} not found`);
    return markType;
};
...
function pluginsFactory$5({ keybindings = defaultKeys$4 } = {}) {
    return ({ schema }) => {
        ...
        return [
            ...
            utils.markInputRule(/(?:^|\s)((?:\*)((?:[^*]+))(?:\*))$/, type),
            keybindings &&
                pm.keymap(utils.createObject([[keybindings.toggleItalic, pm.toggleMark(type)]])),
        ];
    };
}
...
const getTypeFromSchema$2 = (schema) => {
    ...
    utils.assertNotUndefined(markType, `markType ${name$4} not found`);
    return markType;
};
                ...
                new pm.Plugin({
                    props: {
                        handleClick: (view, _pos, event) => {
                            ...
                            const attrs = utils.getMarkAttrs(view.state, getTypeFromSchema$2(schema));
...
function autoLinkInputRule(type) {
    return new pm.InputRule(URL_REGEX, (state, match, start, end) => {
        if (!match[0]) {
            return null;
        }
    ...
    return new pm.Plugin({
        props: {
            handlePaste: function handlePastedLink(view, rawEvent) {
                ...
                const match = utils.matchAllPlus(regexp, text);
    ...
    return new pm.Plugin({
        props: {
            transformPasted: function transformPasted(slice) {
                return utils.mapSlice(slice, (node) => {
                    if (!node.isText) {
                        return node;
                    }
                    const text = node.text;
                    const matches = utils.matchAllPlus(regexp, text);
...
function setLink(from, to, href) {
    href = href && href.trim();
    return utils.filter((state) => isTextAtPos(from)(state), (state, dispatch) => {
...
function createLink(href) {
    return utils.filter((state) => queryIsLinkAllowedInRange(state.selection.$from.pos, state.selection.$to.pos)(state), (state, dispatch) => {
...
function listItemNodeViewPlugin(name) {
    ...
    return core.NodeView.createPlugin({
        name,
        containerDOM: [
            'li',
            {
...
const defaultKeys$3 = {
    toggleDone: utils.browser.mac ? 'Ctrl-Enter' : 'Ctrl-I',
    indent: 'Tab',
    outdent: 'Shift-Tab',
    moveDown: 'Alt-ArrowDown',
    moveUp: 'Alt-ArrowUp',
...
const isValidList = (state) => {
    const type = utils.getNodeType(state, name$3);
    return pmCommands.parentHasDirectParentOfType(type, [
        utils.getNodeType(state, 'bulletList'),
        utils.getNodeType(state, 'orderedList'),
    ]);
};
function specFactory$3() {
    const { toDOM, parseDOM } = utils.domSerializationHelpers(name$3, {
        tag: 'li',
...
function pluginsFactory$3({ keybindings = defaultKeys$3, nodeView = true, } = {}) {
    return ({ schema }) => {
        const type = utils.getNodeType(schema, name$3);
        return [
            keybindings &&
                pm.keymap({
                    [keybindings.toggleDone]: utils.filter(isValidList, updateNodeAttrs(utils.getNodeType(schema, 'listItem'), (attrs) => ({
                        ...attrs,
                    ...
                    ...utils.createObject([
                        ...
                        [keybindings.emptyCut, utils.filter(isValidList, pmCommands.cutEmptyCommand(type))],
                        [
                            keybindings.emptyCopy,
                            utils.filter(isValidList, pmCommands.copyEmptyCommand(type)),
                        ],
...
function moveListItem(dir) {
    return (state, dispatch, view) => {
        const type = utils.getNodeType(state, name$3);
        const isBulletList = pmCommands.parentHasDirectParentOfType(type, [
            utils.getNodeType(state, 'bulletList'),
            utils.getNodeType(state, 'orderedList'),
        ]);
        const move = (dir) => pm.chainCommands(pmCommands.moveNode(type, dir), (state, dispatch, view) => {
        ...
        return utils.filter(isBulletList, move(dir))(state, dispatch, view);
    };
}
...
function insertEmptySiblingList(isAbove = true) {
    return (state, dispatch, view) => {
        const type = utils.getNodeType(state, name$3);
        return pm.chainCommands(utils.filter(isSelectionInsideTodo, utils.insertEmpty(type, isAbove ? 'above' : 'below', true, {
            todoChecked: false,
        })), utils.filter(isValidList, utils.insertEmpty(type, isAbove ? 'above' : 'below', true)))(state, dispatch, view);
    };
}
...
function pluginsFactory$2({ keybindings = defaultKeys$2 } = {}) {
    return ({ schema }) => {
        const type = utils.getNodeType(schema, name$2);
        return [
            pm.wrappingInputRule(/^(1)[.)]\s$/, type, (match) => ({ order: +match[1] }), (match, node) => node.childCount + node.attrs['order'] === +match[1]),
            keybindings &&
                pm.keymap(utils.createObject([[keybindings.toggle, toggleList(type)]])),
        ];
    };
}
function toggleOrderedList() {
    return (state, dispatch, view) => {
        return toggleList(utils.getNodeType(state, name$2), utils.getNodeType(state, 'listItem'))(state, dispatch, view);
    };
}
function queryIsOrderedListActive() {
    return (state) => {
        return pmCommands.parentHasDirectParentOfType(utils.getNodeType(state, 'listItem'), [
            utils.getNodeType(state, name$2),
        ])(state);
    };
}
...
function pluginsFactory$1({ keybindings = defaultKeys$1 } = {}) {
    return ({ schema }) => {
        ...
        return [
            ...
            utils.markInputRule(/(?:^|\s)((?:~~)((?:[^~]+))(?:~~))$/, type),
            keybindings &&
                pm.keymap(utils.createObject([[keybindings.toggleStrike, pm.toggleMark(type)]])),
        ];
    };
}
...
const getTypeFromSchema = (schema) => {
    ...
    utils.assertNotUndefined(markType, `markType ${name} not found`);
    return markType;
};
...
function pluginsFactory({ keybindings = defaultKeys } = {}) {
    return ({ schema }) => {
        ...
        return [
            ...
            utils.markPasteRule(/~([^~]+)~/g, type),
            keybindings
                ? pm.keymap(utils.createObject([[keybindings.toggleUnderline, pm.toggleMark(type)]]))
                : undefined,
        ];
    };
}
...
Object.defineProperty(exports, 'doc', {
    enumerable: true,
    get: function () { return core.doc; }
});
Object.defineProperty(exports, 'editorStateCounter', {
    enumerable: true,
    get: function () { return core.editorStateCounter; }
});
Object.defineProperty(exports, 'history', {
    enumerable: true,
    get: function () { return core.history; }
});
Object.defineProperty(exports, 'paragraph', {
    enumerable: true,
    get: function () { return core.paragraph; }
});
Object.defineProperty(exports, 'text', {
    enumerable: true,
    get: function () { return core.text; }
});
...
exports.link = link;
exports.listItem = listItemComponent;
